...
def add_shadow(base, box, radius=46, blur=32, opacity=110):
    x, y, w, h = box
    shadow = Image.new("RGBA", (w, h), (0,0,0,0))
    ImageDraw.Draw(shadow).rounded_rectangle([0,0,w,h], radius=radius, fill=(0,0,0,opacity))
    shadow = shadow.filter(ImageFilter.GaussianBlur(blur))
    base.alpha_composite(shadow, (x, y+6))

def draw_controls(draw, base_y, center_x):
    gap = 120
    color = (0,0,0)
    s = 22

    # Shuffle
    draw.line([(center_x-2*gap-20, base_y-8),(center_x-2*gap+15, base_y-8)], width=4, fill=color)
    draw.polygon([(center_x-2*gap+15, base_y-14),(center_x-2*gap+28, base_y),(center_x-2*gap+15, base_y+14)], fill=color)

    # Previous
    draw.polygon([(center_x-gap+20,base_y-15),(center_x-gap-30,base_y),(center_x-gap+20,base_y+15)], fill=color)
    draw.rectangle([center_x-gap+28, base_y-15, center_x-gap+36, base_y+15], fill=color)

    # Play
    r = 30
    draw.ellipse([center_x-r, base_y-r, center_x+r, base_y+r], outline=color, width=4)
    draw.polygon([(center_x-10, base_y-18),(center_x-10, base_y+18),(center_x+18, base_y)], fill=color)

    # Next
    draw.polygon([(center_x+gap-20,base_y-15),(center_x+gap+30,base_y),(center_x+gap-20,base_y+15)], fill=color)
    draw.rectangle([center_x+gap-36, base_y-15, center_x+gap-28, base_y+15], fill=color)

    # Repeat
    draw.arc([center_x+gap*2-24, base_y-20, center_x+gap*2+24, base_y+20], start=210, end=20, width=4, fill=color)
    draw.polygon([(center_x+gap*2+26, base_y-2),(center_x+gap*2+10, base_y-10),(center_x+gap*2+6, base_y+10)], fill=color)
